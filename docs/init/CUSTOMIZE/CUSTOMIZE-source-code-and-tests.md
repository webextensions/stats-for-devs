...
- [`./frontend/src/App/LibraryDemo/LibraryDemo.tsx`](../../../frontend/src/App/LibraryDemo/LibraryDemo.tsx) -
  renders the stub component inside the demo app (`node --run start`), importing the library from
  source so the dev build / HMR sees library edits without a rebuild (customizing the demo app
  itself: [CUSTOMIZE-frontend-build.md](./CUSTOMIZE-frontend-build.md)).
